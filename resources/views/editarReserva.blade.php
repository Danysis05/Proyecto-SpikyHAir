...
<form action="{{ route('reservas.update', $reserva) }}" method="POST">
    ...
    <select id="servicio_id" name="id_servicio" required>
        ...
        @foreach($servicios as $servicio)
            <option value="{{ $servicio->id }}" {{ $reserva->id_servicio == $servicio->id ? 'selected' : '' }}>
                {{ $servicio->nombre }}
            </option>
        @endforeach
    </select>
    ...
    <input type="time" id="hora" name="hora" value="{{ $reserva->hora }}" required>

    <button type="submit">Actualizar Reserva</button>
    <a href="{{ route('crudReservasUsuario') }}" id="back">Regresar</a>
</form>
